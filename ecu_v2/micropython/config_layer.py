# Profile schema, sanitize/clamp, CRC, atomic save/recovery.
#
# This file is a copy of ecu/config_layer.py kept here verbatim so that
# the v2 hybrid firmware preserves the exact same on-disk and on-the-wire
# config representation as v1. The dash firmware does not need to change.
#
# Keep this in lockstep with ecu/config_layer.py. If the v1 file changes,
# update this one too.
...
def _normalize_map(xs, ys, x_min, x_max, y_min, y_max):
    ...
    i = 0
    while i < n:
        x = _clamp(int(xs[i]), x_min, x_max)
        y = _clamp(int(ys[i]), y_min, y_max)
        pairs.append((x, y))
        i += 1
    if len(pairs) < 2:
        return None, None
    pairs.sort(key=lambda p: p[0])
    out_x, out_y, last_x = [], [], None
    for p in pairs:
...
def sanitize_profile(profile):
    ...
    adv_x, adv_y = _normalize_map(base.get("advance_map_rpm", []),
                                   base.get("advance_map_cd", []),
                                   300, 20000, -1000, 4500)
    if adv_x is None:
        d = default_profile(); adv_x, adv_y = d["advance_map_rpm"], d["advance_map_cd"]
    dwell_x, dwell_y = _normalize_map(base.get("dwell_map_rpm", []),
                                       base.get("dwell_map_us", []),
                                       300, 20000, 900, 4000)
    if dwell_x is None:
        d = default_profile(); dwell_x, dwell_y = d["dwell_map_rpm"], d["dwell_map_us"]
...
def profile_crc16(profile):
    return crc16_ccitt(ujson.dumps(sanitize_profile(profile)).encode("utf-8"))


def _wrap_clean(clean):
    return {"profile": clean, "crc16": crc16_ccitt(ujson.dumps(clean).encode("utf-8"))}


def _verify_blob(blob):
    if not isinstance(blob, dict): return None
    profile, crc = blob.get("profile"), blob.get("crc16")
    if profile is None or crc is None: return None
    clean = sanitize_profile(profile)
    if (int(crc) & 0xFFFF) != (crc16_ccitt(ujson.dumps(clean).encode("utf-8")) & 0xFFFF):
        return None
    return clean
...
def load_profile_file(path):
    try:
        with open(path, "r") as f:
            return _verify_blob(ujson.loads(f.read()))
    except Exception:
        return None
...
def load_profile_with_recovery():
    ...
    if active is not None: return active, False
...
def save_profile_pair(profile):
    clean = sanitize_profile(profile)
    if not _atomic_write_clean(BACKUP_PATH, clean): return False
    if not _atomic_write_clean(ACTIVE_PATH, clean): return False
    return True
